...
def convert_species(train, test):
    ...
    for df in [train, test]:
        ...
        df['IsOther'] = (df['Species'] != 'CULEX PIPIENS') * (df['Species'] != 'CULEX PIPIENS/RESTUANS') * (
...
def add_trap_bias(train, test, bias_factor=50.0):
    ...
    for trap in traps:
        ...
        if num == 0:
            ...
            prob = np.min([scipy.stats.hypergeom.sf(wnv - 1, num_total, num_wnv, num),
                           scipy.stats.hypergeom.cdf(wnv, num_total, num_wnv, num)])
...
def add_features(train, test):
    ...
    for df in [train, test]:
        ...
        df['DaysSinceAug1'] = np.array(
            [(datetime.datetime(y, m, d) - datetime.datetime(y, 8, 1)).total_seconds() / (60 * 60 * 24.0)
...
def count_nearby_mult(ds, dist_func, max_days=7, max_lat=0.1, max_lng=0.1, same_trap=False, same_species=False,
                      use_self=True, ignore_count1=False):
    '''
    ...
    nearby_mult = []
    for s, y, d, lat, lng, sp, tr in zip(sp_tr_date, ds['year'], ds['DaysSinceAug1'], ds['Latitude'], ds['Longitude'],
                                         ds['Species'], ds['Trap']):
        cond = (ds['year'] == y) * (ds['DaysSinceAug1'] > (d - max_days - 0.01)) * (
        ds['DaysSinceAug1'] < (d + max_days + 0.01))
        ...
        if len(counts_c) == 0:
            ...
            weights = dist_func(np.abs(d - dsc['DaysSinceAug1']),
                                np.sqrt((lat - dsc['Latitude']) ** 2 + (lng - dsc['Longitude']) ** 2),
...
def calc_mult_combined_probs(test, mult_counts_probs_test, mult_nearby_probs_test, mult_nearby_probs_test2):
    ...
    mult2_pow = 0.5
    counts_test_sp1 = [counts_test.get((1, s[1], s[2], s[3]), 1) for s in
                       sp_tr_date_test]  # counts for species=1 (Pipiens)
    counts_test_sp2 = [counts_test.get((2, s[1], s[2], s[3]), 1) for s in
                       sp_tr_date_test]  # counts for species=2 (PipiensRestuans)
    counts_test_sp3 = [counts_test.get((3, s[1], s[2], s[3]), 1) for s in
                       sp_tr_date_test]  # counts for species=3 (Restuans)
    mult_combined_probs_test = []
    for mn, mn2, mc, s, cnt1, cnt2, cnt3 in zip(mult_nearby_probs_test, mult_nearby_probs_test2, mult_counts_probs_test,
                                                sp_tr_date_test,
        ...
        if counts_test[s] == 1:
            ...
            if (s[0] == 1 or s[0] == 3) and (cnt2 > 1):
                ...
                fac = 1.0
            mult_combined_probs_test.append(
                fac * np.min([2.0, 1.0 + ((mn - 1.0) ** mult1_pow) * mult1_mul]) * (mn2 ** mult2_pow))
        else:
...
def apply_species_coeffs(test, preds, coeffs, species):
    ...
    '''
    return np.array([v * ((1.0 - spc) + spc * coeffs[y]) for v, y, spc in
                     zip(preds, np.array(test['year']), np.array(test[species] == 1))])
    ...
    for year in outbreaks_daily_factors.keys():
        for month in outbreaks_daily_factors[year].keys():
            for day in outbreaks_daily_factors[year][month].keys():
                ...
                outbreaks_daily_factors[year][month][day] = outbreaks_daily_factors[year][month][day] / \
                                                            outbreaks_daily_weights[year][month][day]
...
def get_outbreaks(test):
    ...
    # Optimized using LB feedback:
    outbreaks_08 = [(2008, 6, 1, 0.01), (2008, 6, 16, 0.01), (2008, 7, 1, 0.01), (2008, 7, 16, 0.9), (2008, 8, 1, 1.0),
                    (2008, 8, 16, 1.0), (2008, 9, 1, 1.0), (2008, 9, 16, 1.0), (2008, 10, 1, 0.01)]
    outbreaks_10 = [(2010, 6, 1, 0.01), (2010, 6, 16, 0.01), (2010, 7, 1, 0.01), (2010, 7, 16, 1.5), (2010, 8, 1, 1.2),
                    (2010, 8, 16, 1.1), (2010, 9, 1, 1.0), (2010, 9, 16, 0.8), (2010, 10, 1, 0.01)]
    outbreaks_12 = [(2012, 6, 1, 0.01), (2012, 6, 16, 0.01), (2012, 7, 1, 3.0), (2012, 7, 16, 2.5), (2012, 8, 1, 2.5),
                    (2012, 8, 16, 0.5), (2012, 9, 1, 0.1), (2012, 9, 16, 0.1), (2012, 10, 1, 0.01)]
    outbreaks_14 = [(2014, 6, 1, 0.01), (2014, 6, 16, 1.0), (2014, 7, 1, 1.0), (2014, 7, 16, 0.5), (2014, 8, 1, 1.0),
                    (2014, 8, 16, 1.8), (2014, 9, 1, 1.8), (2014, 9, 16, 0.9), (2014, 10, 1, 0.1)]
...
def apply_outbreaks(test, preds, outbreaks):
    ...
    return np.array(
        [pr * outbreaks_daily_factors[y][m][d] for pr, y, m, d in zip(preds, test['year'], test['month'], test['day'])],
        dtype=np.float32)
...
def get_mini_outbreaks(test):
    ...
    # Optimized using LB feedback:
    mini_outbreaks = [(2012, 7, 17, 0.8), (2012, 7, 20, 1.2), (2012, 7, 23, 0.8), (2012, 7, 26, 1.2),
                      (2012, 7, 29, 0.8), (2012, 8, 1, 1.2)]
...
def apply_mini_outbreaks(test, preds, mini_outbreaks):
    ...
    for year, month, day, factor in mini_outbreaks:
        od = datetime.datetime(year, month, day)
        preds = np.array([pr * (
        1.0 if np.abs((datetime.datetime(y, m, d) - od).total_seconds() / (60 * 60 * 24.0)) > 1.5 else factor)
...
def apply_geo_outbreaks(test, preds, geo_outbreaks):
    ...
    for geo_year, geo_biases in geo_outbreaks.iteritems():
        preds = np.array([v * (np.average(geo_biases,
                                          weights=[1.0 / ((1e-3) + test_geo_dists[tg][i]) ** dist_pow for tg in
                                                   range(3)]) if y == geo_year else 1.0)
...
def apply_trap_outbreaks(test, preds, trap_outbreaks):
    ...
    for trap, yearly_facs in trap_outbreaks:
        ...
        preds = np.array(
            [v * (yearly_facs[y] if tr == trap else 1.0) for v, y, tr in zip(preds, test['year'], test['Trap'])])
    return preds
...
if __name__ == "__main__":
    ...
    mult_dist_func = lambda dist_days, dist_geo, species1, species2: (1.0 / (1.0 + dist_days)) * (
    1.0 / (1.0 + 100000 * dist_geo))
    nearby_mult_test = count_nearby_mult(test, mult_dist_func, max_days=35, max_lat=None, max_lng=None, same_trap=False,
                                         same_species=True)
    mult_nearby_probs_test = np.array(
        [mult_entries_func(1.0 + m / 50.0) / mult_entries_func(1.0 + num_mosq_per_entry_count[1] / 50.0) for m in
         nearby_mult_test])
    ...
    nearby_mult_test2 = count_nearby_mult(test, mult_dist_func2, max_days=14, max_lat=None, max_lng=None,
                                          same_trap=True, same_species=True)
    mult_nearby_probs_test2 = np.array(
        [mult_entries_func(1.0 + m / 50.0) / mult_entries_func(1.0 + num_mosq_per_entry_count[1] / 50.0) for m in
         nearby_mult_test2])
    ...
    mult_combined_probs_test = calc_mult_combined_probs(test, mult_counts_probs_test,
                                                        mult_nearby_probs_test,
                                                        mult_nearby_probs_test2)
